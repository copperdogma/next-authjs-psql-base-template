...
export type AriaRole = 'alert' | 'alertdialog' | 'application' | 'article' | 'banner' | 
 ...
 */
export function getElementLocator(page: Page, element: ElementSelector, options: SelectorOptions = {}): Locator {
...
export async function waitForElementToBeVisible(
  ...
  try {
    ...
    const fallbacks = [
      ...
      ...(element.role ? [typeof element.role === 'string' ? 
          `[role="${element.role}"]` : 
          `[role="${element.role.name}"]`] : []),
      ...(element.tag ? [`${element.tag}`] : []),
      // Very generic fallbacks if all else fails
      'header', 'main', 'footer', 'nav', 'div.container', 'div.mx-auto'
    ];
