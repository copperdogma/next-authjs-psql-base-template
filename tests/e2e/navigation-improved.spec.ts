...
test.describe('Navigation and Layout', () => {
  ...
  test('responsive design should work on mobile viewport', async ({ page }) => {
    ...
    const mobileMenuButton = await page.getByRole('button', { name: /menu/i }).isVisible()
      .catch(() => false);
    ...
    if (mobileMenuButton) {
      ...
      await expect(menuButton, 'Mobile menu button should be visible on small viewport').toBeVisible();
      
      // Click the menu button to open mobile navigation
      await menuButton.click();
      
      // Wait for mobile menu to appear and verify it's visible
      const mobileMenu = await page.locator('[role="menu"], [aria-label*="mobile"]').isVisible()
        .catch(() => false);
        
      if (mobileMenu) {
        const menu = page.locator('[role="menu"], [aria-label*="mobile"]');
        await expect(menu, 'Mobile menu should be visible after clicking menu button').toBeVisible();
        ...
        expect(visibleLinks, 'Nav links should be visible after clicking menu button').toBeGreaterThan(0);
      }
    } else {
