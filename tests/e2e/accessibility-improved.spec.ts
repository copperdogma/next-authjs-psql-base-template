...
test.describe('Accessibility Tests', () => {
  ...
  test('accessibility on mobile viewport', async ({ page }) => {
    ...
    if (violations.length > 0) {
      ...
      const violationsByType = violations.reduce<Record<string, AxeViolation[]>>((acc, violation) => {
  ...
  test.skip('keyboard navigation should be possible', async ({ page }) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const isFocusVisible = await page.evaluate(() => {
        ...
        const isVisible = activeElement.offsetWidth > 0 && 
        ...
        const hasFocusStyles = 
          ...
          (computedStyle.outlineColor && 
            computedStyle.outlineColor === 'rgb(37, 99, 235)') || // --focus-ring in light mode
          (computedStyle.outlineColor && 
            computedStyle.outlineColor === 'rgb(59, 130, 246)');  // --focus-ring in dark mode
