...
- [Automated Error Checking (for AI)](#automated-error-checking-for-ai)
...
## Automated Error Checking (for AI)

To support AI-driven development and ensure robust testing, we have implemented automated checks for both browser console errors and server-side log errors during E2E test runs.

### Browser Console Error Checking

- **Mechanism:** Playwright's `test` object has been extended (`tests/e2e/utils/test-base.ts`). It automatically listens for `console.error` messages and uncaught page exceptions (`pageerror`) during the execution of every E2E test.
- **Failure:** If any relevant browser console error or uncaught exception is detected during a test, that specific test will **fail immediately** after its execution.
- **Output:** The Playwright test report will clearly indicate the failed test and include the specific browser console error messages or exception details that caused the failure.

### Server Log Error Checking

- **Mechanism:** The primary `npm run test:e2e` command now executes a wrapper script (`scripts/run-e2e-with-checks.js`).
  1. This script first **clears** the server log file (`./logs/e2e-server-run.log`) generated by the test server (`npm run dev:test`).
  2. It then runs the standard E2E test suite using `start-server-and-test`.
  3. **After** the Playwright tests complete (regardless of pass/fail), the script scans the `./logs/e2e-server-run.log` file for lines matching predefined error patterns (e.g., ERROR, WARN, Exception, etc.).
- **Failure:**
  - If the Playwright tests themselves fail (including failures due to browser console errors), the overall `npm run test:e2e` command will fail.
  - If the Playwright tests pass, but the script detects errors in the server log file, the overall `npm run test:e2e` command will **still fail**.
- **Output:** If server log errors are detected, the script will print a prominent error message and the specific offending log lines to the console before exiting with a non-zero status code.

### Importance for AI Agents

These automated checks provide clear, immediate, and actionable failure signals:

- **Failed Test:** Indicates a problem likely related to the test's specific actions or the component under test (check Playwright report for browser errors).
- **Failed Script (after tests pass):** Indicates a server-side issue occurred during the test run (check console output for server log errors).

This eliminates the need for the AI to manually parse logs after successful test runs and ensures that subtle browser or server errors don't go unnoticed.
