...
export class QueryOptimizer {
   ...
   */
  static selectOnly<T extends Record<string, any>>(fields: (keyof T)[]): Partial<Record<keyof T, true>> {
   ...
   */
  static cursorPagination<T extends Record<string, any>>(options: {
  ...
  static selectWithCounts<T extends Record<string, any>, R extends Record<string, any>>(
    ...
    return {
      ...select,
      _count: {
        select: countFields.reduce((countSelect, field) => {
